...
```{python}
fig, ax = plt.subplots(1, 2, figsize=(len(features)*7, 7))
sq.pl.spatial_scatter(
    adata,
...
fig, axes = plt.subplots(1, len(features), figsize=(len(neighborhood_match_test_features)*5, 5))
for feature, ax in zip(neighborhood_match_test_features, axes):
    title = feature.replace("_", " ").capitalize()
    sq.pl.spatial_scatter(adata, color=feature, shape=None, size=20, cmap="YlOrBr", title=title, ax=ax, fig=fig, use_raw=False)
    ax.set_axis_off()
```
...
sq.pl.spatial_scatter(
    adata,
    color="cluster",
    palette=ListedColormap(cmap_discrete.colors[:len(np.unique(adata.obs["cluster"]))]),
    shape=None,
    library_id="spatial",
    title="Clusters",
    ax=ax,
...
```{python, eval = FALSE}
sq.gr.interaction_matrix(adata, "cluster", normalized=False, connectivity_key="spatial")
df_join_counts_multivariate = pd.DataFrame(adata.uns["cluster_interactions"], columns=np.unique(adata.obs["cluster"]), index=np.unique(adata.obs["cluster"]))
df_join_counts_multivariate
```
