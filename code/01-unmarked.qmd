...
We can use a permutation test to test the inhomogeneoity assumption. In this scenarion we split the patterns into quadrats and compare the estimatied functions between the quadrats. It has to be noted that this test highly depends on the definition of the quadrats.
...
#### Edge corrections for spatial metrics
...
In the case that a spatial pattern is known or suspected to be inhomogeneous, we have to take this into account for our analysis. Biological samples display inhomogeneity very often, therefore this analysis is preferred over the homogeneous alternatives. In the sample that we have chosen, Ependymal cells and OD mature cells are inhomogeneous by eye. Inhomogeneous alternatives can be calculated via:
...
The different y-axis are all variants of edge corrections as explained above.
...
p <- ggplot(K_list_scaled, aes(x=r, y=iso, col= type))+
  geom_line()+
...
p <- ggplot(L_list_scaled, aes(x=r, y=iso, col= type))+
  geom_line()+
...
functional.pca.pp <- function(df){
  ...
  #fdapace::CreateModeOfVarPlot(fpca_obj)
}
...
p <- ggplot(G_list, aes(x=r, y=rs, col= type))+
  geom_line()+
...
plot(ecdf(nndist(ppls$Ependymal)), col = 'red', xlim = c(0,350) )
plot(ecdf(nndist(ppls$`OD Mature`)), add=TRUE, col = 'blue')
plot(ecdf(nndist(ppls$Microglia)), add=TRUE, col = 'green')
```
...
p <- ggplot(F_list, aes(x=r, y=rs, col= type))+
  geom_line()+
...
p <- ggplot(J_list, aes(x=r, y=rs, col= type))+
  geom_line()+
...
The $G$ function indicates that the nearest-neighbour distances remain relatively small for increasing values of $r$ for Ependymal cells. Microglial cells increase along the dashed poisson curve. OD mature cells (what are these curves actually telling us?)
