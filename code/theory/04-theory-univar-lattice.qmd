...
res <- spdep::globalG.test(x = counts(sfe)[features[1],], 
                    listw = weights_neighbourhoods_binary)
res 
```

The result of global Getis-Ord $G$ for the gene `r features[[1]]` is `r round(res$estimate[[1]], digits = 5)`, the associated p-values is `r res$p.value[[1]]`. This means that gene `r features[[1]]` shows `r if(res$statistic[[1]]>0){'a hot spot'}else{'a cold spot'}`
