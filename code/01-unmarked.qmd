...
testingCSR <- function(
  ...
  if(method == 'quadrat'){
    if(mark == TRUE){
      test.result <- lapply(split(pp), quadrat.test, 5, alternative="regular", method="MonteCarlo")
    }
    else{
      test.result <- quadrat.test(unmark(pp), 5, alternative="regular", method="MonteCarlo")
    }
   }
  ...
  #perform a clark evans test for individual marks or for the entire pointprocess
     ...
     if(mark == TRUE){
      test.result <- lapply(split(pp), clarkevans.test, alternative="regular")
    }
    else{
      test.result <- clarkevans.test(unmark(pp), alternative="regular")
    } 
  }
