...
Under the null $I$ takes the value $-1/(n-1)$ in expectation. This value is close to $0$ for large $n$. A value higher than $\mathbb{E}(I) = -1/(n-1)$ indicates spatial auto-correlation. Negative auto-correlation is not so easy to interpret [1]. The implementation below is a Monte Carlo simulation approach to define a null distribution to test against. 
