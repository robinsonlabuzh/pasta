...
For the moment the functions of `spatialFDA` are in this script below
...
plotMetricPerFov <- function(metric_df, theo = FALSE, correction = NULL, x = NULL, image_id = NULL) {
    p <- ggplot(metric_df, aes(x = .data[[x]], y = .data[[correction]], group = factor(image_id), colour = factor(selection))) +
        geom_line() +
        facet_grid(selection~ID) +
        theme_minimal() +
...
extractMetric <- function(df, selection, fun, marks = NULL, r_seq = NULL, by = NULL) {
    ...
    # SG: this handles the case when we do cross functions for the same type
        ...
        length(selection) > 1) {
      print("case2")
      # use pp insead ot pp_sel (mark connection problem)
...
calcCrossMetricPerFov <- function(spe,
  ...
  res_l <- apply(l, 1, function(x)
    calcMetricPerFov(
      spe, x, subsetby, fun, marks, r_seq, by, ncores
    ))
...
plotMetricPerFov <- function(metric_df, theo = FALSE, correction = NULL, x = NULL, image_id = NULL) {
    p <- ggplot(metric_df, aes(x = .data[[x]], y = .data[[correction]], group = factor(.data[[image_id]]), colour = factor(.data[['sample_id']]))) +
        geom_line() +
