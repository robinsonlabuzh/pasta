...
we will try to apply functional PCA to retrieve the main trends in these individual curves. The idea of functional PCA is the same as for ordinary PCA just applied to the concept of functions. For the $n$ functions above functional PCA will recover the main trends in the data

```{r}
functional.pca.pp <- function(df){
  df_fdob <- df %>% as.matrix()
  #remove theo column - we want only the actual estimations in there
  df_fdob <- df_fdob[,-ncol(df_fdob)]
  #remove r column - this has to be provided independently to the FPCA
  df_fdob <- df_fdob[,-ncol(df_fdob)]
  #number of columns
  N <- ncol(df_fdob)
  #number of rows
  M <- nrow(df_fdob)
  #the x values at which all the curves were evaluated
  s <- df$r
  ...
  fdapace::CreatePathPlot(fpca_obj, subset=1:2, main = 'Estimated Paths for first two curves'); grid()
}
...
p <- ggplot(G_inhom_list, aes(x=r, y=bord, col= type))+
  geom_line()+
...
p <- ggplot(F_inhom_list, aes(x=r, y=bord, col= type))+
  geom_line()+
...
p <- ggplot(J_inhom_list, aes(x=r, y=bord, col= type))+
  geom_line()+
...
The concept of spacing is not only usable in point pattern analysis but more broadly in any spatial context.
