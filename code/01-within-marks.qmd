...
permutation_test <- function(pp, mark, split, minpoints) {
  ...
  tesselation <- quantess(unmark(pp_sel), "x", 2)
...
lapply(c("Microglia", "OD Mature", "Ependymal"), function(x) permutation_test(pp, x, split = 3, minpoints = 10))
```

The p-value of the test for local scaling for both Microglia and OD mature cells is $<0.05$ which indicates that the assumption of local scaling is rejected. Therefore, the distribution of microglia cells and oligodendrocytes is not a scaled version of an overall distribution pattern.

The p-value of the test for inhomogeneity `for both Microglia and OD mature cells is $>0.05$ indicating that the assumption of correlation stationarity is not rejected. In this case we can use the inhomogeneous version of the K-function which assumes correlation stationarity.

For ependymal cells however, the p-values for both the local scaling test and the inhomogeneity test are $>0.05$ which means that both the correlation stationarity assumption and the local scaling assumption can't be rejected. [ME: Does this make sense? Or is this just an artifact?] 
...
```{r, cache=FALSE}
val <- quadrat.test(ppls$`OD Mature`, 5, alternative="regular", method="MonteCarlo")
val
```
...
```{r, cache=FALSE}
clarkevans.test(ppls$`OD Mature`)
```
...
As we have seen above in the test for homogeneity in oligodendrocytes, the assumptions of homogeneity are not given in our data. Therefore, we will have to use the inhomogeneous alternatives of the metrics instead.
...
As seen by our permutation tests we have some grounds to believe that the pattern of ependymal cells is locally scaled. Therefore, we will analyse this pattern using a locally scaled K and L function. 

We see, that again our ependymal cells are very far from the poisson process line, indicating strong clustering among ependymal cells. 
