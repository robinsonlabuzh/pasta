...
#as a histogram this fits a mixture model with 2 (?) gamma distributions one representing cluster feature and one representing noise
nearestNeighbourCleaning(ppls$Ependymal, mark=FALSE)
nearestNeighbourCleaning(ppls$Microglia, mark=FALSE)
...
nndistances_k10_microglia <- nndist(ppls$Microglia, k = 5)
hist_ependymal <- hist(nndistances_k10_ependymal, breaks = 60, plot = FALSE)
hist_microglia <- hist(nndistances_k10_microglia, breaks = 60, plot = FALSE)
...
plot(hist_ependymal, col = c1, main = "Histogram of Ependymal and Microglial Cells", freq=FALSE)
lines(density(gamma_mm_comp2_ependymal),col='blue')
plot(hist_microglia, col = c2, add = TRUE, legend=c("Ependymal", "Microglia"),freq=FALSE)
lines(density(gamma_mm_comp2_microglia),col='red')
legend(400, 95, legend=c("Ependymal", "Microglia"),
       col=c(c1,c2), lty=1:2, cex=0.8)

# nndistances_k10_ependymal_df <- as.data.frame(nndistances_k10_ependymal)
# nndistances_k10_microglia_df <- as.data.frame(nndistances_k10_microglia)
# 
# p <- ggplot(nndistances_k10_ependymal_df, aes(x = nndistances_k10_ependymal, alpha = 0.2)) +
#   geom_histogram(fill=c1, binwidth = 10) + 
#   geom_histogram(data = nndistances_k10_microglia_df,aes(x = nndistances_k10_microglia,fill = c2, alpha = 0.2), binwidth = 10)
# p
```
...
testingCSR(ppls$Ependymal, method='hopkins-skellam')

testingCSR(ppls$Microglia, method='hopkins-skellam')
```
