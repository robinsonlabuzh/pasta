...
Since we are calculating several local statistics potentially from the same observations, problems of multiple testing arise. We correct for this issue with standard adjustments, such as Benjamini Hochberg correction (https://rss.onlinelibrary.wiley.com/doi/10.1111/j.2517-6161.1995.tb02031.x)

```{r}
#calculate local Moran's I and then correct for multiple testing using Benjamini-Hochberg correction
locM <- localmoran(x = sfe_tissue$nGenes, listw = weights_neighbourhoods) |> p.adjust("BH")

colData(sfe_tissue) <- cbind(sfe_tissue, locM)

tmap::tm_shape(sfe_tissue) + tm_fill()
```
#### Implementation wiht `Voyager` 
