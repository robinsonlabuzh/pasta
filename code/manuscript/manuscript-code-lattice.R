...
Nrgn <- logcounts(spe)["Nrgn",] |> as.matrix() |> as.data.frame() 
...
st_buffer(st_geometry(spsf), dist = 80) |> poly2nb(snap = 10) |> nb2listw() -> direct_neigbours
...
pLocEff_direct <- ggplot() + 
  ...
  scale_color_distiller(palette = "Spectral", limits = c(-2,8)) +
  theme_void()
...
pDetected <- plotVisium(spe, fill = "Nrgn", assay = "logcounts",
...
colGraph(sfe, "knn10") <- findSpatialNeighbors(sfe, method = "knearneigh",
                                              dist_type = "idw", k = 10, 
                                              style = "W")
...
bbox_use <- st_bbox(c(xmin = 8000, xmax = 14000, ymin = 161000, ymax = 166000))
...
pKnn <- pKnn + scale_fill_distiller(palette = "RdYlBu",
                            limits = c(-2, 13)) +
...
pPoly <- pPoly + scale_fill_distiller(palette = "RdYlBu",
                             limits = c(-2, 13)) +
...
ggsave("outs/localMoranOverview.pdf", fig2, width = 8, height = 7)


###

data.frame(knn = localResult(sfe, "localmoran", "KRT17")[,"Ii"],
...
pknncont <- data.frame(knn = localResult(sfe, "localmoran", "KRT17")[,"Ii"],
             cont = localResult(sfePoly, "localmoran", "KRT17")[,"Ii"]) |> 
  ggplot(aes(x = knn, y = cont)) + 
    geom_point(size = 0.5) +
  geom_abline(intercept = 0, slope = 1, col = "cornflowerblue") + 
  theme_light() +
  labs(x = "Local Moran's I \n(10 Nearest Neighbours)",
       y = "Local Moran's I \n(Contiguos Neighbours)") +
  coord_fixed(ratio = 1)
