...
set.seed(123)
x1 <- rnorm(400,rep(10*1:5,each=40),sd=7)
#y1 <- rep(0,200)
y1 <- rnorm(400,rep(10*sample.int(5,5),each=40),sd=9)

x2 <- rnorm(400,rep(10*1:5,each=40),sd=5)
y2 <- rnorm(400,rep(10*sample.int(5,5),each=40),sd=5)

# distance to the 5th NN transformed via sqrt
nnd1 <- sqrt(BiocNeighbors::findKNN(cbind(x1,y1), k = 15)$distance)
nnd2 <- sqrt(BiocNeighbors::findKNN(cbind(x2,y2), k = 15)$distance)
...
cowplot::plot_grid(nrow=2,
  cowplot::plot_grid(nrow=1,
    ggplot(data.frame(x=x1,y=y1), aes(x,y)) + geom_point() + ggtitle("Scenario A"),
    ggplot(data.frame(x=x2,y=y2), aes(x,y)) + geom_point() + ggtitle("Scenario B")),
  ggplot(d, aes(distance, colour=scenario)) + geom_density() + xlab("Mean of NN distance k=1-15"))

ks.test(nnd1_mean,nnd2_mean)
CDFt::CramerVonMisesTwoSamples(nnd1_mean,nnd2_mean)
```
...
```{r, cache=FALSE}
set.seed(123)
#sub sample two columns via transpose at random from the dataframe (k vs. distances)
index_k <-  sample(ncol(nnd1), 2)
...
fasano.franceschini.test::fasano.franceschini.test(nnd1_sub,nnd2_sub,nPermute=1000)
```
...
plot(ecdf(nnd1_mean), main='Scenario A and B mean curves', col = 'red', xlim = c(min(c(nnd1_mean,nnd2_mean)),max(nnd1_mean)))
plot(ecdf(nnd2_mean), add=TRUE, col = 'blue')

#calculate the eCDF of the two processes
ecdf1_fun <- ecdf(nnd1_mean)
ecdf2_fun <- ecdf(nnd2_mean)

#number of discrete steps
P <- 400

#calculate the sequence over which to discretise, has to be the min and max over all samples
seq <- seq(from = min(c(nnd1_mean,nnd2_mean)), max(c(nnd1_mean,nnd2_mean)), length.out=P)
...
s_obs <- max(ecdf.diffs)


permutation.test <- function(s_obs,pp1,pp2,nsim){
  distribution=c()
  for(i in 1:nsim){
...
permutation.test(s_obs, nnd1_mean, nnd2_mean, 1000)
ks.test(nnd1_mean,nnd2_mean)
```
