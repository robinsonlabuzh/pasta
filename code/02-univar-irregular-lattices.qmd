...
[MR: does it ever make sense to have variance of counts? The variance will likely be tied to the mean. I guess it depends where it is used.]
...
In general, a spatial autocorrelation measure has the following elements: a function, $f(x_i, x_j)$, of the values at locations $x_i$ and $x_j$ and a weight matrix, $w_{ij}$. The function $f$ relates the measured values $x_i$ and $x_j$ and the weight matrix $w_{ij}$ builds in the spatial locations of $x_i$ and $x_j$. If $x_i$ and $x_j$ are not neighbours, the element of the weights matrix is 0 (i.e., $w_{ij} = 0$). In general, the spatial autocorrelation has the form of a double sum over all locations: $\sum_i \sum_j f(x_i,x_j) w_{ij}$.

[MR: notation above: is it safe to say that $i$ and $j$ are the locations and $x_i$ and $x_j$ are the measurements at those locations? Or, are $x_i$ and $x_j$ the locations?]

[MR: what is a canonical form of $f$? .. or will we see this below?]
...
One of the challenges when working with irregular lattice data is the construction of a neighbourhood graph. The main question is, what to consider as neighbours, as this will affect downstream analyses. Various methods exist to create neighbours, such as contiguitiy based neighbours, graph-based neighbours, distance based neighbours or higher order neighbours. Segmentation of individual cells is notoriously difficult [MR: add citation for this?], therefore we cannot simply rely on neighbours based on the segmentation results [MR: why not?] and the construction of contiguity-based neighbours is therefore not ideal, also as it would neglect the influence of more distant neighbours.
...
[MR: I think in general, it's best to give citations as you go through the motivation than to just give a dumped listed of references for the reader.]
...
[MR: not sure if these notes above are intended to still be there, but I would prefer to see the doc written in a normal linear paragraph style.]
...
The global Moran's I coefficient is a measure of spatial autocorrelation, defined as:

$$I = \frac{n}{\sum_i\sum_j w_{ij}} \frac{\sum_i\sum_j w_{ij}(y_i - \hat{y})(y_j - \hat{y})}{\sum_i (y_i - \hat{y})^2}, $$

where $y_i$ is the value of the variable of interest for location $i$ [MR: perhaps we should use this notation above!], $\hat{y}$ is the mean of $y_i$ [MR: why hat and not the usual bar for mean?] and $w_{ij}$ is the spatial weight between locations $i$ and $j$. The expected value is close to $0$ for large $n$ ($\mathbb{E}(I) = -1/(n-1)$), whereas a value higher than indicates spatial auto-correlation. Negative values indicate negative auto-correlation.
...
We can also use the `spdep::moran.mc` function to calculate the Moran's I coefficient. This function uses a Monte Carlo simulation to calculate the P-value. 
...
We can see both genes have a positive Moran's I coefficient and a highly significant P-value. The expected value is $\mathbb{E}(I) = -1/(n-1)$ which is for large $N$ close to 0. Positive and significant values indicate clustering of similar like values [MR: is there a better way to express "clustering of similar like values"? I don't really know what that means.]. It is important to note that this could be both at the high or low end of the values. Negative values indicate clustering of alternating values, i.e., gives a measure of spatial heterogeneity. It is also important to note that the result is dependent on the neighbourhood matrix. Different neighbourhood matrices will give different results. To compare Moran's I coefficients between different values, we need to use the same neighbourhood matrix.

## Global's Geary's coefficient

Geary's $C$ is a different measure of global autocorrelation and is very closely related to Moran's $I$. However, it focuses on spatial dissimilarity. Geary's $C$ is defined by

$$
C = \frac{(n-1) \sum_i \sum_j w_{ij}(y_i-y_j)^2}{2\sum_i \sum_j w_{ij}\sum_i(y_i-\bar{y})^2}
$$

where $n$ is the number of locations, $i,j$ are different locations, $y_i$ is the value of the variable of interest for location $i$, $\bar{y}$ is the mean of $y_i$ and $w_{ij}$ is the spatial weight between locations $i$ and $j$.  The interpretation is opposite to Moran's $I$: a value less than $1$ indicates positive auto-correlation whereas a value greater than $1$ represents negative auto-correlation.
...
The local Getis-Ord $G_i$ statistic quantifies the weighted concentration of points within a radius $d$ and in a local region $i$, according to:

$$
G_i(d) = \frac{\sum_{j \neq i } w_{ij}(d)x_j}{\sum_{j \neq i} x_j}.
$$

There is a variant of this statistic, $G_i^*(d)$, which is the same as $G_i(d)$ except that the contribution when $j=i$ is included in the term. Importantly, $G_i(d)$ is scale-invariant but not location-invariant, meaning that the subdivision into the $n$ subregions matters for the computation of the local statistic [MR: what does "matters" mean in this sentence?].
...
The LOSH should be interpreted in combination with the local Getis-Ord $G_i^*$ statistic. The $G_i^*$ quantifies the local mean of the variable of interest, while $H_i$ quantifies the local variance. This table provided by Ord and Getis (2012) summarizes the interpretation of the combination of $G_i^*$ and $H_i$.
...
The local methods presented above should always be interpreted with care, since we face the problem of multiple testing when calculating them for each cell. Also the P-values calculated are pseudo P-values [MR: what are pseudo P-values?] and are calculated for each neighbour separately. Moreover, the presented methods should mainly serve as exploratory measures to identify interesting regions in the data. Multiple processes can lead to the same pattern, thus from identifying the pattern we cannot infer the underlying process. Indication of clustering does not explain why this occurs. There can be true contagion where clustering is the result of spatial interaction and apparent contagion where clustering is the cause of spatial heterogeneity, e.g., local similarity is created by different spatial structures. [MR: I'm not sure I understand the example here.]
