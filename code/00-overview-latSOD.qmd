...
- All `Python` examples were written by [Malte Kuehl](https://www.au.dk/en/malte.kuehl@clin.au.dk)
...
There also exist methods to detect spatial autocorrelation of categorical variables. As we have no categorical measurements in this dataset, we will perform non-spatial clustering using Leiden clustering on the PCA space.

```{r}
# Run PCA on the sample
sfe <- runPCA(sfe_tissue)
...
```{python}
sq.pl.spatial_scatter(adata, color="cluster", palette=ListedColormap(cmap_discrete.colors[:len(np.unique(adata.obs["cluster"]))]), library_id="spatial", title="Clusters", shape=None, size=55)
plt.gca().set_axis_off()
```
